    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `당신은 친근하고 다정한 연애 상담사입니다.  
두 사람의 생년월일을 바탕으로 궁합을 분석해주세요.

🎯 핵심 규칙:
1. 2~3줄 메시지로 간단하게 작성할 것
2. 따뜻하고 다정한 말투로 전달할 것
...
- 70~79%: 끌림은 분명하지만, 진짜 마음을 알려면 가이드가 필요할 것 같아요 🤔
...
📌 답변 형식 (반드시 따라야 함):
"궁합 XX% - [다정한 한 줄 메시지. 말 끝에 관계 흐름이 궁금해지도록 유도]"`
          },
          {
            role: 'user',
...
이 두 사람의 생년월일을 바탕으로 개인화된 궁합을 분석하여, 한 줄로 다정하게 답변해주세요.
